export async function onRequest(context) {
  ...
  try {
    ...
    // Build dynamic UPDATE query for tracks table
    const fields = [];
    const values = [];
    
    // Track table fields (artist fields removed - now handled via track_artists)
    ...
    // Update tracks table if there are fields to update
    if (fields.length > 0) {
      values.push(id);
      ...
      for (let i = 0; i < updates.artists.length; i++) {
        ...
        if (artist.id) {
          ...
          `).bind(
            id, 
            artist.id, 
            artist.is_primary ? 1 : 0, 
            artist.display_order !== undefined ? artist.display_order : i
          ).run();
        }
      }
    }
    
    // Handle main_artist_id and featured_artists_ids separately (alternative format)
    ...
    // Parse artists JSON and add backward compatibility fields
