export async function onRequest(context) {
  ...
  try {
    ...
    await env.DB.prepare(`
      UPDATE compilations SET status = 'published', updated_at = CURRENT_TIMESTAMP WHERE id = ?
    `).bind(id).run();
    
    return new Response(JSON.stringify({ success: true, status: 'published' }), { headers });
